...
4. Import, name the profile with the entry and exit, and allow the OS VPN configuration.
...
- [ ] Provider recovery console opens (KiwiVM for BandwagonHost).
...
- [ ] Reality works before an optional exit is added.
- [ ] Direct exit matches the selected entry VPS.
- [ ] If a SOCKS5/ISP exit is selected, standalone testing matches its purchased IP.
- [ ] If chained, the final Reality path matches that exit on two checks.
