...
“客户端”就是你在电脑或手机上点击连接的应用。服务器配置完成后，Agent 会在你的电脑本地生成一份连接信息或二维码，你把它导入应用即可。

连接信息相当于密码，只在自己的设备之间传递。不要把它粘贴到聊天、网盘、在线二维码网站或公共订阅。你也不用手写里面的服务器地址、端口和密钥。

## 选择客户端

先在下面找到自己的设备。拿不准下载哪个文件时，把设备型号和下载页面截图给 Agent，让它指出文件名；不要随便试多个来路不明的安装包。
